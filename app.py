...
def load_table(engine, table_name, limit=10000):
    ...
    try:
        # Экранируем имя таблицы на случай пробелов или спецсимволов
...
class Preprocessor:
    ...
    def normalize_numeric(df):
        ...
        if len(num_cols) > 0:
            # Заполняем пропуски нулями временно для нормализации
...
def plot_missing_heatmap(df_original):
    ...
    # Берем только первые 200 строк и 20 столбцов для скорости отрисовки
...
def main():
    ...
    with st.sidebar:
        ...
        user = st.text_input("Пользователь", "" if db_type == "SQLite" else "postgres")
        
        # ИСПРАВЛЕННАЯ СТРОКА НИЖЕ:
        password = st.text_input("Пароль", "" if db_type == "SQLite" else "", type="password")
    ...
    if 'df_raw' in st.session_state:
        ...
        # Вкладки
        tab1, tab2, tab3 = st.tabs(["⚙️ Настройка анализа", "📈 Результаты", "💾 Экспорт"])
        ...
        with tab1:
            ...
            with col2:
                check_missing = st.checkbox("🕳️ Обработать пропуски", value=True)
                ...
                if check_outliers:
                    ...
                    if outlier_method == "Isolation Forest":
                        ...
                        if len(df_ml) < 5000:
                            ...
                            st.warning("LOF слишком медленный, используем Isolation Forest")
                            df_ml = OutlierDetector.detect_outliers_iforest(df_ml, contamination)
...
if __name__ == "__main__":
    main()def load_table(engine, table_name, limit=10000):
    """Загружает данные из таблицы в DataFrame."""
    try:
        query = f"SELECT * FROM \"{table_name}\" LIMIT {limit}"
        df = pd.read_sql(query, engine)
...
class Preprocessor:
    ...
    def normalize_numeric(df):
        ...
        if len(num_cols) > 0:
            ...
            # Но лучше это делать после разделения на train/test, здесь упрощаем для демо
            df_temp = df[num_cols].fillna(0)
            df[num_cols] = scaler.fit_transform(df_temp)
        return df
...
def generate_report(df, dupe_groups):
    ...
    # 3. Пропуски (Проверяем по исходному датафрейму, но здесь мы уже в обработанном,
    # поэтому логику пропусков лучше показывать отдельно в статистике или тепловой карте)
...
def main():
    ...
    with st.sidebar:
        ...
        database = st.text_input("Имя базы данных / Путь к файлу", "test_quality_with_defects.db" if db_type == "SQLite" else "")
        user = st.text_input("Пользователь", "" if db_type == "SQLite" else "postgres")
        password = st.text_input("Пароль", type="password", "" if db_type == "SQLite" else "")
