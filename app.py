...
def detect_outliers_numeric(df, contamination=0.1):
    ...
    clf.fit(X_scaled)
    df['is_outlier'] = clf.labels_   # -1 = аномалия, 0 = норма
...
def generate_report(df, dupe_groups):
    ...
    if 'duplicate_group' in report.columns:
        report.loc[report['duplicate_group'] != -1, 'problem_type'] += 'дубликат;'
    if 'is_outlier' in report.columns:
        report.loc[report['is_outlier'] == -1, 'problem_type'] += 'аномалия;'
    # Пропуски считаем по исходным данным, здесь просто метка
...
def main():
    ...
    if 'engine' in st.session_state and 'tables' in st.session_state:
        ...
        # Дополнительно: исключаемые колонки для поиска дубликатов (например, id)
        exclude_cols = st.multiselect("Колонки, которые НЕ учитывать при поиске дубликатов (например, id, timestamp)",
                                      st.session_state['tables'] if 'tables' in st.session_state else [],
                                      default=[])
    ...
    if 'df_raw' in st.session_state:
        ...
        with tab2:
            if st.session_state.get('analysis_complete', False):
                ...
                n_out = (df_res['is_outlier'] == -1).sum() if 'is_outlier' in df_res else 0
                d3.metric("Аномалий (Isolation Forest)", n_out)
                n_miss = df_raw.isnull().any(axis=1).sum()
                d4.metric("Строк с пропусками (исходно)", n_miss)
                ...
                with st.expander("🔎 Примеры найденных проблем"):
                    if 'is_outlier' in df_res and (df_res['is_outlier'] == -1).any():
                        st.write("**Примеры аномальных записей:**")
                        st.dataframe(df_res[df_res['is_outlier'] == -1].head(10))
                    if dupe_groups:
